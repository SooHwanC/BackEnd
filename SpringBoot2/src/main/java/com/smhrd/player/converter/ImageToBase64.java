...
public class ImageToBase64 implements PlayerImageConverter<File, String> {
	// 이미지 파일을 byte 문자열 형태로 변환할때
	// base64(이미지 기본 인코딩)을 활용해서 인코딩하는 클래스

	@Override
	public String convert(File f) throws IOException {

		// 1. 파일 가져오기 -> commons-io 라이브러리
		// commons-io : 파일, 디렉토리 조작, 입출력, 파일 필터링 등과 같은 기능 제공
		
		// readFileToByteArray : 파일 가져와서 byte 배열 형태로 읽는 메서드
		// 픽셀을 byte로 변경, 픽셀이 여러개라서 배열 형태로 반환
		byte[] file = FileUtils.readFileToByteArray(f);
		
		// 2. byte배열 -> base64로 이미지 인코딩
